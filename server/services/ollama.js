...
function formatCampaignPlan(planSummary) {
  ...
  if (planSummary.main_quest_title) {
    sections.push(`MAIN QUEST: ${planSummary.main_quest_title}
...
This campaign has a pre-generated plan. Use this as your guide for the overarching story:

${sections.join('\n\n')}

CRITICAL RULES FOR USING THIS PLAN:
1. The NPCs listed above are from the player's backstory - USE THEM, do not invent generic replacements
2. World events will unfold according to the timeline - the world is ALIVE and changes regardless of player action
3. Weave the main quest naturally into the narrative - don't force it
4. The themes should emerge through story, not exposition
5. This plan is your guide, not a script - adapt to player choices while maintaining narrative coherence
=== END CAMPAIGN PLAN ===`;
}
...
- After an NPC asks a question, STOP. End your response immediately.
- Do NOT continue talking after a question - wait for the player to answer
...
- If the player attacks a friendly NPC: the NPC reacts realistically (fear, defense, fleeing, calling guards)
...
1. ONLY use NPCs explicitly named in the scene - NO inventing new characters
2. After ANY NPC asks a question, STOP your response immediately
3. ONE question maximum per response
4. Use the EXACT names given for NPCs - "Durnan" not "the bartender"
5. Stay in second person ("you") for the player character`;
}
...
`;
  ...
  const campaignFoundationPrompt = !isPublishedModule ? `
