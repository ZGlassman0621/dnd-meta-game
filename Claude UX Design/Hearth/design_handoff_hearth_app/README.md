...
**A · Greeting (entry).** Single hero card (bg-card, 3px gold bar). Opus orb + "OPUS"; serif 33px headline ("Every campaign begins as a single sentence. Tell me yours."); serif 18px sub with gold-italic emphasis. Three input groups:
- **"Who walks into this story?"** — 2 selectable cards: the existing character (crest + name) or **"Start from the world"** (dashed globe tile). (Ownership can be character-first OR world-first.)
- **"Or begin from a seed I've already drafted"** — 2 cards: a backstory-drafted seed ("The high passes", ink-blue tag) and "Surprise me" (Opus's choice).
- **"What do you want to play?"** — a large serif textarea (italic placeholder) with a primary **"Begin with Opus"** send button bottom-right.
Choosing a character/seed or sending the prompt → switches to Compose.
...
- **Right rail** (`.rail`, sticky `top:78px`):
  - **"The shape of it"** panel ("Set by Opus" badge): **Scope** segmented control (One-shot / **Short arc** / Open-ended); **Genre & tone** selectable chips; **Setting** value tile (MapPin); **"Who starts beside you"** party (the hero pill + a dashed "Add a companion" that inserts an ally pill); **Difficulty & the DM** (an "Inherit from Settings" toggle + a value tile); **Lines & veils** button ("3 set"). *(These dials are interactive but the "Change/Edit" links and a full companion/veil picker are stubbed — to be built.)*
