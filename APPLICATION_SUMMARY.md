...
- **Opus 4.5 generation**: Comprehensive campaign plans generated by Claude Opus 4.5
- **Plan components**:
...
- **Two-stage AI pipeline**:
  - **Opus 4.5** — Generates campaign plans (world building, NPCs, quest arcs, factions)
  - **Sonnet 4** — Runs interactive gameplay sessions (narration, combat, dialogue)
...
2. Player rolls initiative → AI establishes turn order
3. On player's turn: "Roll to attack" with target AC
4. Player rolls → AI narrates hit/miss, requests damage roll if hit
5. Full turn-by-turn combat until resolution
...
- `POST /api/campaign/:id/plan/generate` - Generate plan with Opus 4.5
...
| AI Providers | Claude API (Opus 4.5 + Sonnet 4), Ollama (local) |
...
**Core Routes**:
- `/api/character` - Character CRUD, progression, parsed backstory
...
- `POST /:sessionId/message` - Send player action (returns narrative + downtime/recruitment detection)
...
**Opus 4.5** builds the world (campaign plans, NPCs, factions, quest arcs) → **Sonnet 4** runs gameplay (narration, combat, dialogue with enforced D&D 5e rules).
...
2. **AI DM sessions** with Sonnet 4 — full combat mechanics (initiative, attack rolls, damage), NPC question pausing, narrative consistency
