...
- **Campaign description priority**: The campaign description is the #1 rule — main quest fulfills the campaign vision, backstory characters are woven in as supporting elements
...
#### Campaign Plan → DM Session Data Flow
...
- **Two-stage AI pipeline**:
  - **Claude Opus** — All world building and content generation (campaign plans, backstory parsing, NPC/quest/location/companion generation, living world events, adventures)
  - **Claude Sonnet** — Runs interactive DM sessions only (narration, combat, dialogue)
  - Ollama — Local fallback with offline support for all tasks
...
**Combat Mechanics**:
1. Player declares attack → AI emits `[COMBAT_START: Enemies="..."]` and STOPS
...
5. Player rolls → AI narrates hit/miss, requests damage roll if hit
6. Full turn-by-turn combat until `[COMBAT_END]` clears the tracker

**Campaign Plan Integration**:
- When a campaign plan exists (generated by Opus), the `campaignFoundationPrompt` tells Sonnet to USE the plan — not invent its own story
...
- Cleanup order: unassign characters → clear location refs → delete narrative queue → delete journeys → delete world events → delete factions → delete quests → delete locations → delete campaign
...
- **Auto-Pipeline**: On submission, runs create → assign → parse backstory → generate campaign plan
...
- **Main bundle**: ~971 kB (244 kB gzipped) — down from 1,321 kB
...
- **Adventure** — Main gameplay with message display and action input
- **Downtime** — Embedded Downtime component for rest, work, and training activities
- **Stats** — Embedded MetaGameDashboard for campaign statistics and time management
...
```
Create character → Write backstory (optional) → Create Campaign → Auto-pipeline (assign + parse + generate) → Play Now → Adventure!
```
...
```
Play button (home) or Navigate to AI DM → Start session → Opus campaign plan injected into Sonnet's system prompt → Interactive roleplay with enforced combat/NPC rules → Switch to Downtime/Stats tabs mid-session (changes injected back into AI context) → Session ends → Claim rewards → Campaign notes updated
```

### Flow 3: Downtime Management
```
Do downtime (standalone or via DM session tab) → Benefits applied to character → If mid-session, context injected into AI conversation → Advance time → Events feed into next DM session
```
...
**Claude Opus** builds the world (campaign plans, backstory parsing, NPC/quest/location/companion generation, living world events, adventures) → **Claude Sonnet** runs interactive DM sessions (narration, combat, dialogue with enforced D&D 5e rules). Both models use alias IDs (no date suffix) so they auto-update to the latest version.

### Core Gameplay Loop
1. **Create a character** with backstory → **Create a campaign** (auto-pipeline: assign + parse backstory + generate world plan) → **Play** via the green Play button
2. **AI DM sessions** with Claude Sonnet — full combat mechanics (initiative, attack rolls, damage), NPC question pausing, narrative consistency
3. **Downtime** between sessions (or mid-session via tabs) — rest, train, craft, work for gold
4. **Campaign plan** guides the story — Sonnet follows Opus's quest arc, NPCs, and world state
