...
- **Multi-provider architecture**:
  - Claude API (Anthropic) - Primary AI option (Opus 4.5 for generation, Sonnet 4 for sessions)
...
```
Play button (home) or Navigate to AI DM → Start session with campaign plan context → Interactive roleplay → Switch to Downtime/Stats tabs as needed → Session ends → Claim rewards → Campaign notes updated
```
