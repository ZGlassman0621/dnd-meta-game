...
| 23 | **Prelude mentor seeds the primary campaign's `mentor_imprints` table** — if a mentor emerges in the prelude, they're a canonical mentor imprint when the primary campaign begins. <br/>*Note (2026-04-29): Design-only until Phase 2 (Prelude → Primary transition). The `mentor_imprints` table and seeding service are not yet implemented in code. Rule remains in the design spec; implementation gates on Phase 1's reframe output.* | Round 2 |
...
| 29 | **Theme commitment ceremony at Ch3 wrap-up — character choices drive departure.** Throughout Ch1-2, the PC's character-shaping choices fire `[THEME_HINT]` markers (existing emergence system); the chapter-weighted tally accumulates signal. At Ch3 wrap-up (after the irreversible-act beat), the AI emits `[THEME_COMMITMENT_OFFERED]` and the UI renders a "Choose Your Path" card with the leading theme + 3 alternatives + a wildcard drawn from talents/cares + full-list "Other" + "See where it goes" defer. Player commits via `POST /api/prelude/:id/commit-theme` → server writes `characters.prelude_committed_theme`. At Ch4 opening, the committed theme drives the departure TYPE (soldier → enlistment, acolyte → pilgrimage, etc. — 21 themes mapped in `preludeThemeService.THEME_DEPARTURE_MAP`); tone preset modulates the FEEL. This keeps the player in control of their path while honoring played behavior. Deferring falls back to the trajectory winner + arc-plan departure_seed. | v1.0.77 |
