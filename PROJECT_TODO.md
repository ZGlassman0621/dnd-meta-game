...
- [ ] **Validate v1.0.96 cache fix in real Opus playtest** — see [`triage/prose-quality-triage.md`](triage/prose-quality-triage.md). Status: cache architecture fix shipped (~$1.55 → ~$0.85 per Opus session predicted). Need real session logs to confirm 95% cache hit rate in production-cadence play.
...
- **v1.0.96 (2026-04-26)** — Prose-quality diagnostic + prompt cache architecture fix. Opus is the validated narrative lever. Cache hit rate ~55% → ~95% predicted on Opus sessions; per-session cost ~$1.55 → ~$0.85.
