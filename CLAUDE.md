...
**Chunk 5 main creator (batch 2 in flight)** — `client/src/components/creator/` houses the rebuilt 8-step creator: `CharacterCreatorV2.jsx` (shell), `creatorPrimitives.jsx` (Field/Stepper/WizardHead/WizardFoot/Eyebrow), `CelebrationCard.jsx` (handoff-mode primitive shared by Steps 2/3/5), `Step1Identity.jsx`, `Step2Ancestry.jsx`, `Step3Theme.jsx`, `Step4ClassCalling.jsx`. Steps 5–8 + persistence + home-page wiring + Screen 2 land in batch 3. The new creator is reachable via `?creator=v2` query-param preview (with `&handoff=1` for the fixture); existing `CharacterCreationWizard.jsx` stays as the live path until 5.K wires the new one in. Editorial aesthetic: EB Garamond + Inter + JetBrains Mono via Google Fonts in `client/index.html`; design tokens + structural classes in `client/src/styles/creator-theme.css`, all scoped under `.creator-v2` so the rest of the app's dark slate gradient is untouched. Per Decision 6: editorial-only, no aesthetic toggle.
