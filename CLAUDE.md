...
**Chunk 5 content data files** (`client/src/data/`): `themeGoldModifiers.js`, `themePersonalityPrompts.js`, `themeIdealsPrompts.js`, `themeBondsPrompts.js`, `themeFlawsPrompts.js`, `themeBackstoryMoments.js` — 618 PM-authored entries transcribed verbatim from spec §7. Prompt files (personality / ideals / bonds / flaws) are `{ themeId: [{ text, alignment }] }` with always-visible 9-square alignment indicators (Decision 3). Backstory moments are `{ themeId: [string] }` — bare strings, no alignment per Decision 4 (events ≠ commitments). Bracketed placeholders in prompt/moment text (e.g. `[the village that raised you]`) are intentional player-fill-in invitations.
