...
**Chunk 5 main creator (batch 3 checkpoints in flight)** — `client/src/components/creator/` houses the rebuilt 8-step creator: `CharacterCreatorV2.jsx` (shell), `creatorPrimitives.jsx` (Field/Stepper/WizardHead/WizardFoot/Eyebrow), `CelebrationCard.jsx` (handoff-mode primitive shared by Steps 2/3), `BumpCelebrationCard.jsx` (Step 5's count-aware variant with per-bump assignment dropdowns), `Step1Identity.jsx`, `Step2Ancestry.jsx`, `Step3Theme.jsx`, `Step4ClassCalling.jsx`, `Step5AbilityScores.jsx`, `Step6Equipment.jsx`. Step 7 + Step 8 + Submit + persistence land in checkpoint 2; home-page wiring + Screen 2 + cleanup land in checkpoint 3. The new creator is reachable via `?creator=v2` query-param preview (with `&handoff=1&fixture=verena|single-bump|hermit` for fixture variants exercising singular/plural bump phrasing + all three gold modifier display variants); existing `CharacterCreationWizard.jsx` stays as the live path until 5.K wires the new one in. Editorial aesthetic: EB Garamond + Inter + JetBrains Mono via Google Fonts in `client/index.html`; design tokens + structural classes in `client/src/styles/creator-theme.css`, all scoped under `.creator-v2` so the rest of the app's dark slate gradient is untouched. Per Decision 6: editorial-only, no aesthetic toggle.
