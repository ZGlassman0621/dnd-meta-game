...
- Level-up endpoint (`POST /api/character/level-up/:id`) as of v1.0.7: pre-mutation validation (422 if subclass required but missing), feat-instead-of-ASI support (asiChoice.type = 'asi' or 'feat'), all writes in `db.transaction('write')` with rollback on error. `LevelUpPage.jsx` is the single level-up UI surface (LevelUpModal deleted); CharacterManager's button delegates to parent via `onShowLevelUp` prop
